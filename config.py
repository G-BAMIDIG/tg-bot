...
# ---- Database ----
# Render's free plan wipes local files (including a SQLite file) on every
# restart/redeploy, so all persistent data now lives in a real Postgres
# database (e.g. a free Supabase/Neon project) instead. DB_PATH is kept
# only as a fallback local-dev value, unused once DATABASE_URL is set.
DB_PATH = _anchored_path(os.getenv("DB_PATH"), "vpnbot.db")
DATABASE_URL = os.getenv("DATABASE_URL")
if not DATABASE_URL:
    raise RuntimeError(
        "DATABASE_URL is not set. Add your Supabase/Neon Postgres connection "
        "string to .env (local) or Render's Environment tab (production)."
    )
