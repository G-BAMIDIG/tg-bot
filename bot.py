...
# ---------------- Admin: message one specific user by their Telegram ID ----------------
...
def admin_message_user(message):
    ...
    if len(parts) != 3 or not parts[2].strip():
        bot.reply_to(message, t("admin_message_usage", "en"))
        return
    try:
    ...
    try:
        ...
        bot.reply_to(message, t("admin_message_sent", "en", user_id=target_id))
    except Exception as e:
        bot.reply_to(message, t("admin_message_failed", "en", user_id=target_id, error=str(e)))
...
def _setup_command_menu():
    ...
    bot.set_my_commands(
        [
            ...
            _types.BotCommand("message", "Message one user by their ID"),
